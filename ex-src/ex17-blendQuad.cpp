...
void blendQuad(char **args)
{
	...
	while (true)
	{
		...
		{
		case 'a':// a: ブレンド結果を蓄積（元画像に反映）
			imageLine = result.clone();
			imageTmp = result.clone();
			srcA = result.clone();

			break;
		case 'b':// b, B: blur の程度（回数等）を小さく調整
			blurNumber--;
			if(blurNumber > 0)
			{
				for(int i = 1; i <= blurNumber; i++)
				{
					cv::blur(mask,mask,cv::Size(2 * blurNumber + 1, 2 * blurNumber + 1));
				}
			}
			...
			cv::imshow("Mask Image", mask);
			break;
		case 'B':// b, B: blur の程度（回数等）を大きく調整
			blurNumber++;
			if(blurNumber > 0)
			{
				for(int i = 1; i <= blurNumber; i++)
